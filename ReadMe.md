QTHungryDogsCustomer  
=============  
  
Das Projekt ***QTHungryDogsCustomer*** ist eine Vorlage fuer die Erstellung von datenzentrierten Anwendungen. Ausgehend von dieser Vorlage koennen neue Anwendungen, sogenannte Domain-Projekte, erstellt und erweitert werden.  
...
4. [Verwendung der Vorlage](#verwendung-der-Vorlage)  
   ...
   2. [Abgleich mit dem QTHungryDogsCustomer](#abgleich-mit-dem-QTHungryDogsCustomer)  
...
## Template  
Die Struktur vom 'QTHungryDogsCustomer' besteht aus unterschiedlichen Teilprojekten und diese in einer Gesamtloesung (im Kontext von Visual Studio ist das eine Solution) zusammengefasst. Eine Erlaeuterung der einzelnen Projekte, deren Typ und die Abhaengigkeit finden sie in der folgenden Tabelle:  
...
|**QTHungryDogsCustomer.Logic**|Dieses Projekt beinhaltet den vollstaendigen Datenzugriff, die gesamte Geschaeftslogik und stellt somit den zentralen Baustein des Systems dar.|Library|CommonBase|
|**QTHungryDogsCustomer.WebApi**|In diesem Projekt ist die REST-Schnittstelle implementiert. Diese Modul stellt eine API (Aplication Programming Interface) fuer den Zugriff auf das System ueber das Netzwerk zur Verfuegung.|Host|CommonBase, QTHungryDogsCustomer.Logic|
|**QTHungryDogsCustomer.ConApp**|Dieses Projekt dient als Initial-Anwendung zum Erstellen der Datenbank, das Anlegen von Anmeldedaten falls die Authentifizierung aktiv ist und zum Importieren von bestehenden Daten. Nach der Initialisierung wird diese Anwendung kaum verwendet.|Console|CommonBase, QTHungryDogsCustomer.Logic|
|**QTHungryDogsCustomer.AspMvc**|Diese Projekt beinhaltet die Basisfunktionen fuer eine AspWeb-Anwendung und kann als Vorlage fuer die Entwicklung einer einer AspWeb-Anwendung mit dem QTHungryDogsCustomer verwendet werden.|Host|CommonBase, QTHungryDogsCustomer.Logic|
|**QTHungryDogsCustomer.WpfApp**|Diese Projekt beinhaltet die Basisfunktionen fuer eine Wpf-Anwendung und kann als Vorlage fuer die Entwicklung einer einer Wpf-Anwendung mit dem QTHungryDogsCustomer Framework verwendet werden.|Host|CommonBase, QTHungryDogsCustomer.Logic|
|**QTHungryDogsCustomer.XxxYyy**|Es folgen noch weitere Vorlagen von Client-Anwendungen wie Angular, Blazor und mobile Apps. Zum jetzigen Zeitpunkt existiert nur die AspMvc-Anwendung. Die Erstellung und Beschreibung der anderen Client-Anwendungen erfolgt zu einem spaeteren Zeitpunkt.|Host|CommonBase, QTHungryDogsCustomer.Logic|
